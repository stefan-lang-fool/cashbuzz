...
import { Hint, LineMarkSeries, XYPlot, XAxis, YAxis, HorizontalGridLines, AreaSeries } from 'react-vis';
...
import { getCurrentDateFormat, formatDateWithoutYear } from '@/utils/format';
...
const Liquidity = ({ accounts }: Props): JSX.Element => {
  ...
  const [liquidityHintText, setLiquidityHintText] = useState<string>(t('overview.general.liquidity_chart.hint.value'));
    ...
    const fetchFinanceGroups = async (): Promise<void> => {
      try {
        ...
        const filteredFinanceGroups = data.finance_groups.filter(
          (singleGroup) => ![CATEGORIZATION.FINANCIAL_GROUP_FOR_INCOME_CONSUMER, CATEGORIZATION.FINANCIAL_GROUP_FOR_EXPENSES_CONSUMER].includes(singleGroup.label)
        );
              ...
              >
                ...
                <LineMarkSeries
                  ...
                  onValueMouseOver={ (value): void => {
                    setLiquidityHintText(t('overview.general.liquidity_chart.hint.weekly_value'));
