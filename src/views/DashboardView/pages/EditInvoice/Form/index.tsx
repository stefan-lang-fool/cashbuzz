...
import { Invoice, InvoiceRequestData, Organization } from '@/interfaces';
...
interface Props {
  initialAddress?: { street?: string; codeAndCity?: string; country?: string };
  initialData: Invoice;
...
const Form = ({ initialAddress, initialData, initialOrg, goBack, onUpdate }: Props): JSX.Element => {
  const { t } = useTranslation();
  const classes = styles();

  const [selectedCountry, setSelectedCountry] = useState<null | string>(getCurrentLanguage() === 'en' ? 'Germany' : 'Deutschland');
  const [amount, setAmount] = useState<number | undefined>(initialData.gross_amount);
  const [isLoading, setIsLoading] = useState(false);
  const [options, setOptions] = useState<Organization[]>([]);
  const [currentOrganization, setCurrentOrganization] = useState<Organization | undefined>(initialOrg);
  ...
  const { getValues, register, handleSubmit, errors, setValue, triggerValidation, control, formState } = useForm<InvoiceFormData>({
    ...
    defaultValues: {
      amount: initialData.gross_amount || undefined,
      dueDate: initialData.due_date || format(new Date(), 'yyyy-MM-dd'),
      invoiceNumber: initialData.invoice_ident || '',
      invoiceDate: initialData.invoice_date || format(new Date(), 'yyyy-MM-dd'),
      name: initialOrg?.name || initialData.org_name || '',
      customerNumber: initialData.customer_ident,
      codeAndCity: initialAddress?.codeAndCity || '',
      streetAddress: initialAddress?.street || ''
    }
  });
  ...
  const onSubmit = useCallback(async (data: InvoiceFormData): Promise<void> => {
    ...
    let requestData: InvoiceRequestData = {
      ...
      payment_transaction: initialData.payment_transaction
    };
    ...
    try {
      await APIService.invoices.edit(initialData.id, requestData);
  ...
  }, [amount, currentOrganization, formatAddress, initialData, onUpdate, setAddressFields, t]);
    ...
    <div className={ classes.root }>
      ...
      <form onSubmit={ handleSubmit(onSubmit) }>
        ...
        >
          ...
          >
            <DatePicker
              defaultDate={ parse(initialData.invoice_date, 'yyyy-MM-dd', new Date()) }
          ...
          >
            <DatePicker
              defaultDate={ parse(initialData.due_date, 'yyyy-MM-dd', new Date()) }
