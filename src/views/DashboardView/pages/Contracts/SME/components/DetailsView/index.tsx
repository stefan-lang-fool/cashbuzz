...
const DetailsView = ({ contractId, onBack }: Props): JSX.Element => {
  ...
  const fetchTransactions = useCallback(async (page: number, offLoading = false): Promise<void> => {
    ...
    const { data } = await APIService.transaction.getAllBySubscription(contractId, page, 10);
  ...
  return (
    <div className={ classes.root }>
      <Typography
        ...
        variant="h4"
      >
        { t('contracts.details_view.title') }
      </Typography>
      <div className={ classes.sectionContent }>
        ...
        >
          <Table stickyHeader>
            ...
            <CustomTableBody
              customEmptyText={ t('contracts.details_view.table.no_data') }
      ...
      </div>
      {
        onBack && (
          <Button
            ...
            variant="contained"
          >
            { t('contracts.details_view.back_button') }
          </Button>
        )
      }
    </div>
